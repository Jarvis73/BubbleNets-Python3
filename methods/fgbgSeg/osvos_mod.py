    ...
    with tf.Session(config=config) as sess:
        ...
        while step < max_training_iters + 1:
            ...
            for _ in range(0, iter_mean_grad):
                ...
                image_shape += batch_image[0].shape
                image = np.zeros(image_shape)
                label_shape = (batch_size,)
                label_shape += batch_label[0].shape
                label_shape += 1,
                label = np.zeros(label_shape)
                image[0] = preprocess_img(batch_image[0])
                label[0] = preprocess_labels(batch_label[0])
                for i in range(1, batch_size):
                    image[i] = preprocess_img(batch_image[i])
                    label[i] = preprocess_labels(batch_label[i])
...
def test(dataset, checkpoint_file, result_path, config=None):
    ...
    with tf.Session(config=config) as sess:
        ...
        for frame in range(0, dataset.get_test_size()):
            ...
            # For windows
            scipy.misc.imsave(os.path.join(result_path, curr_frame.split("\\")[-1]), res_np.astype(np.float32))
