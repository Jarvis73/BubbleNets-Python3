...
class GrabCutter(object):
    ...
    def user_bounding_box(self):
        ...
        print(
            "Click and drag bounding box over object to annotate.\nType 'r' to restart, 's' to save, or 'n' for no object.")
        while True:
        ...
        if len(self.refPt) == 2:
            self.roiPt = (min(self.refPt[0][0], self.refPt[1][0]), min(self.refPt[0][1], self.refPt[1][1]),
                          max(self.refPt[0][0], self.refPt[1][0]), max(self.refPt[0][1], self.refPt[1][1]))
    ...
    def initial_grab_cut(self):
        ...
        cv2.grabCut(self.img, self.mask, self.roiPt, self.bgdModel, self.fgdModel, self.initialIterations,
    ...
    def user_refine_cut(self):
        ...
        while refining:
            ...
            print(
                "Click and draw lines over foreground object (hold right click to undo, press +/- to change brush size).\nType 'r' to restart or 's' to save.")
            while True:
            ...
            self.fgDraw = False
            print(
                "Click and draw lines over background elements (hold right click to undo, press +/- to change brush size).\nType 'r' to restart or 's' to save.")
            while True:
                cv2.imshow('User Foreground / Background Refinement', self.drawImg)
                key = cv2.waitKey(20) & 0xFF
                if key == ord('r'):
            ...
            cv2.grabCut(self.img, self.mask, None, self.bgdModel, self.fgdModel, self.refineIterations,
    ...
    def click_and_draw(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            if self.fgDraw:
                self.drawImg[y - self.brushWidth:y + self.brushWidth, x - self.brushWidth:x + self.brushWidth, :] = [0,
                                                                                                                     255,
                                                                                                                     0]
                self.drawMask[y - self.brushWidth:y + self.brushWidth, x - self.brushWidth:x + self.brushWidth] = 1
            else:
                self.drawImg[y - self.bgBrushWidth:y + self.bgBrushWidth, x - self.bgBrushWidth:x + self.bgBrushWidth,
                :] = [0, 0, 255]
                self.drawMask[y - self.bgBrushWidth:y + self.bgBrushWidth,
                x - self.bgBrushWidth:x + self.bgBrushWidth] = 0
            cv2.imshow('User Foreground / Background Refinement', self.drawImg)
        # Add ability to erase marks during refinement process.
            ...
            if self.fgDraw:
                self.drawImg[y - self.brushWidth:y + self.brushWidth, x - self.brushWidth:x + self.brushWidth,
                :] = self.grabCutImg0[y - self.brushWidth:y + self.brushWidth, x - self.brushWidth:x + self.brushWidth,
                     :]
                self.drawMask[y - self.brushWidth:y + self.brushWidth,
                x - self.brushWidth:x + self.brushWidth] = self.mask[y - self.brushWidth:y + self.brushWidth,
                                                           x - self.brushWidth:x + self.brushWidth]
            else:
                self.drawImg[y - self.bgBrushWidth:y + self.bgBrushWidth, x - self.bgBrushWidth:x + self.bgBrushWidth,
                :] = self.grabCutImg0[y - self.bgBrushWidth:y + self.bgBrushWidth,
                     x - self.bgBrushWidth:x + self.bgBrushWidth, :]
                self.drawMask[y - self.bgBrushWidth:y + self.bgBrushWidth,
                x - self.bgBrushWidth:x + self.bgBrushWidth] = self.mask[y - self.bgBrushWidth:y + self.bgBrushWidth,
                                                               x - self.bgBrushWidth:x + self.bgBrushWidth]
            cv2.imshow('User Foreground / Background Refinement', self.drawImg)
