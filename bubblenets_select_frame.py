...
from methods.annotate_suggest.color_hist_frame_select import color_hist_frame_select
from methods.annotate_suggest.ResNet_preprocess import resnet_process_data_dir
from methods.annotate_suggest.BubbleNets_frame_select import BubbleNets_sort
from methods.annotate_suggest.BubbleNets import bn_utils
...
def get_user_annotation(videoDir):
    ...
    while userAnnotating:
        ...
        if userAnnotating:
            ...
            while True:
                if user_select:
                    annotationImageIdx = int(input('What is preferred annotation image index? (' + str(
                        os.path.basename(imageFiles[0])) + '-' + str(
                        os.path.basename(imageFiles[-1])) + ' possible)\n'))
